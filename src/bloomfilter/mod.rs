...
pub trait BloomFilter {
	...
	{
		if self.shape().m != other.shape().m || self.shape.k != other.shape().k {
			return Err( "Shapes do not match" )
		} 
		let selfBuff = self.as_vec();
		let otherBuff = self.as_vec();
		for i in 0..selfBuff.len() {
			if selfBuff[i] & otherBuff[i] != otherBuff[i] {
				return Ok(false);
			}
		}
		Ok(true)
	...
	fn estimate_union( &self,  other : BloomFilterType ) -> f32 {
		...
		let mut count = 0;
		for i in 0..selfBuff.len() {
			let x = selfBuff[i] | otherBuff[i];
			count += x.count_ones();
		}
		return self.shape().estimate( count );
	}
...
impl Proto for SimpleProto {
	...
	fn build( &self, shape : &Shape ) -> BloomFiltertype {
		let mut filter : BloomFilter =  Simple::new( shape );
...
impl Proto for ProtoCollection {
	fn build( &self, shape : &Shape ) -> Simple {
		let mut filter  =  Simple::new( shape );
		self.add_to( &mut filter );
		return filter
	}
